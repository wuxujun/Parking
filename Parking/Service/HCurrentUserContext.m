...
#define NicknameUserDefaultKey          @"user_nickname"
...
-(void)loginWithUserName:(NSString *)username password:(NSString *)password success:(OperationSuccessBlock)successBlock error:(MKNKErrorBlock)errorBlock{
    NSMutableDictionary *params = [@{@"username":username,@"passwd":password} mutableCopy];
    ...
    [self.networkEngine postOperationWithURLString:url params:params success:^(MKNetworkOperation *completedOperation, id result) {
        ...
        NSInteger isSuccess = [[resultDict objectForKey:@"status"] integerValue];
        if (isSuccess==200) {
            myself.uid = resultDict[APP_REQUEST_USER_IDENTITY];
            myself.username = resultDict[APP_REQUEST_USER_MOBILE];
            myself.nickname = resultDict[APP_REQUEST_USER_NICKNAME];
            myself.email=resultDict[APP_REQUEST_USER_EMAIL];
            successBlock(completedOperation,result);
        } else {
            NSString *message = [result objectForKey:@"msg"] ? : @"登录发生错误";
...
-(void)registerWithUserName:(NSString *)username password:(NSString *)password  sms:(NSString *)sms success:(HBOOLBlock)successBlock error:(MKNKErrorBlock)errorBlock{
    NSString* captchaId=[[NSUserDefaults standardUserDefaults] objectForKey:APP_REQUEST_CAPTCHA_ID];
    NSMutableDictionary *params = [@{@"mobile":username,@"passwd":password,APP_REQUEST_CAPTCHA_VALUE:sms,APP_REQUEST_CAPTCHA_ID:captchaId} mutableCopy];
    ...
    [self.networkEngine postOperationWithURLString:url params:params success:^(MKNetworkOperation *completedOperation, id result) {
        ...
        if (isSuccess==200) {
            ...
            NSString *message = [result objectForKey:@"msg"] ? : @"注册发生错误";
            NSError *error = [NSError errorWithDomain:message code:-1 userInfo:nil];
            if (errorBlock != nil) {
                errorBlock(error);
            }
...
-(void)getUserInfo:(NSString *)uid success:(OperationSuccessBlock)successBlock error:(MKNKErrorBlock)errorBlock
{
    __unsafe_unretained HCurrentUserContext *myself = self;
    NSString *url = [NSString stringWithFormat:@"%@user_info",kHttpUrl];
    [self.networkEngine postOperationWithURLString:url params:nil success:^(MKNetworkOperation *completedOperation, id result) {
        NSDictionary *resultDict = result;
        NSInteger isSuccess = [[resultDict objectForKey:@"status"] integerValue];
        if (isSuccess==200) {
            myself.uid = resultDict[APP_REQUEST_USER_IDENTITY];
            ...
            NSString *message = [result objectForKey:@"msg"] ? : @"查询用户信息发生错误";
            NSError *error = [NSError errorWithDomain:message code:-1 userInfo:nil];
            if (errorBlock != nil) {
                errorBlock(error);
            }
...
-(void)setNickname:(NSString *)nickname{
    _nickname = nickname;
    [UserDefaultHelper setObject:_nickname forKey:NicknameUserDefaultKey];
}

-(NSString *)nickname{
    return _nickname ?:[UserDefaultHelper objectForKey:NicknameUserDefaultKey];
}
